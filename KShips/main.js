(function() {
    ...
    var ACHIEVEMENTS = [
        ...
        { id: 'easy_victory', name: 'Easy Victory', desc: 'Win on Easy difficulty', locked: false, check: function(s) { return s.victory && s.difficulty === 'easy'; } },
        { id: 'medium_victory', name: 'Endurance', desc: 'Win on Medium difficulty', locked: true, lockedBy: 'easy_victory', check: function(s) { return s.victory && s.difficulty === 'medium'; } },
        { id: 'dominator', name: 'Dominator', desc: 'Win on Hard difficulty', locked: true, lockedBy: 'medium_victory', check: function(s) { return s.victory && s.difficulty === 'hard'; } },
        { id: 'insanity', name: 'Insanity', desc: 'Win on Insane difficulty', locked: true, lockedBy: 'dominator', check: function(s) { return s.victory && s.difficulty === 'insane'; } },
        { id: 'unlucky', name: 'Unlucky', desc: 'Lose on Easy difficulty', hidden: false, check: function(s) { return !s.victory && s.difficulty === 'easy'; } }
    ];
    var appSettings = {
    ...
    function attachMenuListeners() {
        ...
        var insane = document.getElementById('insane-btn');
        ...
        if (insane) insane.onclick = function(){ startGame('insane'); };
        ...
        if (undo) undo.onclick = undoLastShip;
    ...
    function showAchievements() {
        ...
        for (var i = 0; i < ACHIEVEMENTS.length; i++) {
            ...
            var isLocked = false;
            if (ach.locked && ach.lockedBy) {
                isLocked = appSettings.unlockedAchievements.indexOf(ach.lockedBy) === -1;
            }
            ...
            if (isLocked) {
                desc.textContent = 'Complete previous achievements to unlock';
            } else {
            ...
            status.className = unlocked ? 'achievement-unlocked-label' : 'achievement-locked-label'; 
    ...
    function showStats() {
        ...
        list.innerHTML = '';
        var winRate = (appSettings.gamesWon + appSettings.gamesLost) > 0 ? 
            Math.round((appSettings.gamesWon / (appSettings.gamesWon + appSettings.gamesLost)) * 100) : 0;
        var avgShotsPerGame = appSettings.gamesWon > 0 ? 
            Math.round(appSettings.totalShotsFired / appSettings.gamesWon) : 0;
        var avgShipsPerGame = appSettings.gamesWon > 0 ? 
            (appSettings.totalShipsSunk / appSettings.gamesWon).toFixed(1) : 0;
        var stats = [
            ...
            { label: 'Avg Shots Per Win', value: avgShotsPerGame },
            { label: 'Avg Ships Per Win', value: avgShipsPerGame },
            { label: 'Best Win Streak', value: appSettings.bestWinStreak || 0 },
            { label: 'Current Win Streak', value: appSettings.currentWinStreak || 0 },
            { label: 'Fastest Win', value: appSettings.fastestWin ? appSettings.fastestWin + ' shots' : 'N/A' },
            { label: 'Most Efficient Win', value: appSettings.mostEfficientWin ? appSettings.mostEfficientWin + '% accuracy' : 'N/A' }
        ];
        for (var i = 0; i < stats.length; i++) {
            ...
            var name = document.createElement('div'); 
            name.className = 'achievement-name'; 
            name.textContent = stats[i].label;
            var value = document.createElement('div'); 
            value.className = 'achievement-desc'; 
    ...
    function bindSettingsButtons() {
        ...
        if (animBtn) animBtn.onclick = function() {
            ...
            saveSettings();
            var a = document.getElementById('animations-check'); 
            if (a) a.textContent = appSettings.animationsEnabled ? '✓' : '';
        };
    ...
    function loadSettings() {
        ...
        if (saved) {
            try {
                ...
                appSettings.currentWinStreak = data.currentWinStreak || 0;
                appSettings.bestWinStreak = data.bestWinStreak || 0;
                appSettings.fastestWin = data.fastestWin || null;
                appSettings.mostEfficientWin = data.mostEfficientWin || null;
            } catch (e) {}
        }
    }
    ...
    function saveSettings() {
        try { 
            localStorage.setItem('battleship_settings', JSON.stringify({ 
                ...
                gamesWon: appSettings.gamesWon, 
                gamesLost: appSettings.gamesLost, 
                totalShipsSunk: appSettings.totalShipsSunk, 
                totalShotsFired: appSettings.totalShotsFired, 
                totalHits: appSettings.totalHits,
                currentWinStreak: appSettings.currentWinStreak,
                bestWinStreak: appSettings.bestWinStreak,
                fastestWin: appSettings.fastestWin,
                mostEfficientWin: appSettings.mostEfficientWin
            })); 
        } catch (e) {}
    }
    ...
    function startGame(difficulty) {
        ...
        var lb = document.getElementById('setup-player-label'); 
    ...
    function setupCellHover(e) {
        ...
        clearPreviews(); 
        showShipPreview(row,col,SHIPS[gameState.currentShipIndex].size, gameState.isHorizontal);
    }
            ...
            if (gameState.currentShipIndex < SHIPS.length) { 
                ...
                if (appSettings.gameMode === 'multiplayer') {
                    ...
                    if (appSettings.currentPlayer === 0) {
                        ...
                        gameState.currentShipIndex = 0; 
                        gameState.isHorizontal = true;
                        var lbl = document.getElementById('setup-player-label'); 
                        if (lbl) lbl.textContent = appSettings.playerNames[1];
                        var sb = document.getElementById('start-game-btn'); 
                        if (sb) sb.disabled = true;
                        var ub2 = document.getElementById('undo-btn');
                        ...
                        var sb2 = document.getElementById('start-game-btn'); 
                        if (sb2) sb2.disabled = false;
                        updateStatus('All ships placed for both players. Start Battle when ready.'); 
                        updateShipInfo();
                    }
                } else {
                    var sb3 = document.getElementById('start-game-btn'); 
                    if (sb3) sb3.disabled = false;
    ...
    function updateShipInfo() {
        ...
        if (!el) return;
        if (gameState.currentShipIndex < SHIPS.length) { 
            el.textContent = SHIPS[gameState.currentShipIndex].name + ' (' + SHIPS[gameState.currentShipIndex].size + ' cells)'; 
        } else { 
    ...
    function undoLastShip() {
        if (gameState.currentShipIndex === 0) return;
        gameState.currentShipIndex--;
        var lastShip = gameState.playerShips.pop();
        if (lastShip) {
            for (var i = 0; i < lastShip.positions.length; i++) {
                var pos = lastShip.positions[i];
        ...
        if (sb) sb.disabled = true;
        var ub = document.getElementById('undo-btn');
        if (ub) ub.disabled = (gameState.currentShipIndex === 0);
    }
    ...
    function randomPlacement() {
        ...
        for (var i = 0; i < SHIPS.length; i++) {
            ...
            while (!placed && attempts < 500) {
                ...
                if (canPlaceShip(gameState.playerBoard,row,col,SHIPS[i].size,isH)) { 
                    placeShip(gameState.playerBoard,gameState.playerShips,row,col,SHIPS[i],isH); 
                    placed = true; 
                }
                attempts++;
            }
    ...
    function placeEnemyShips() {
        gameState.enemyShips = []; 
        gameState.enemyBoard = createEmptyBoard();
        var shipsToPlace = SHIPS;
        if (gameState.difficulty === 'insane') {shipsToPlace = [
                { name: 'Carrier', size: 5 },
                { name: 'Battleship', size: 4 }
            ];
        }
        for (var i = 0; i < shipsToPlace.length; i++) {
            ...
            while (!placed && attempts < 1000) {
                ...
                var col = Math.floor(Math.random() * GRID_SIZE);
                var isH = Math.random() < 0.5;
                if (canPlaceShip(gameState.enemyBoard,row,col,shipsToPlace[i].size,isH)) { 
                    placeShip(gameState.enemyBoard,gameState.enemyShips,row,col,shipsToPlace[i],isH); 
                    placed = true; 
                }
                attempts++;
            }
        ...
        setTimeout(function() {
            ...
            if (gameState.enemyBoard[row][col].ship !== null) {
                var statsNow = { 
                    ...
                    hitRate: Math.round((gameState.hits / gameState.shotsFired) * 100), 
    ...
    function calculateAIProbability() {
        ...
        for (var i = 0; i < SHIPS.length; i++) {
            var ship = SHIPS[i];
            var found = false;
            for (var j = 0; j < gameState.playerShips.length; j++) {
                if (gameState.playerShips[j].name === ship.name && isShipSunk(gameState.playerShips[j])) {
                    found = true;
                    break;
                }
            }
        ...
        for (var s = 0; s < remainingShips.length; s++) {
            var shipSize = remainingShips[s].size;
            for (var row = 0; row < GRID_SIZE; row++) {
                for (var col = 0; col < GRID_SIZE; col++) {
                    if (canPlaceShipForAI(gameState.playerBoard, row, col, shipSize, true)) {
                        for (var i = 0; i < shipSize; i++) {
                            probMap[row][col + i]++;
                        }
                    }
                    if (canPlaceShipForAI(gameState.playerBoard, row, col, shipSize, false)) {
                        for (var i = 0; i < shipSize; i++) {
                            probMap[row + i][col]++;
                        }
                    }
        ...
        for (var r = 0; r < GRID_SIZE; r++) {
            for (var c = 0; c < GRID_SIZE; c++) {
                if (r > 0 && gameState.playerBoard[r-1][c].hit && gameState.playerBoard[r-1][c].ship) {
                    probMap[r][c] += 50;
                }
                if (r < GRID_SIZE-1 && gameState.playerBoard[r+1][c].hit && gameState.playerBoard[r+1][c].ship) {
                    probMap[r][c] += 50;
                }
                if (c > 0 && gameState.playerBoard[r][c-1].hit && gameState.playerBoard[r][c-1].ship) {
                    probMap[r][c] += 50;
                }
                if (c < GRID_SIZE-1 && gameState.playerBoard[r][c+1].hit && gameState.playerBoard[r][c+1].ship) {
                    probMap[r][c] += 50;
                }
            }
        }
    ...
    function canPlaceShipForAI(board, row, col, size, isHorizontal) {
        for (var i = 0; i < size; i++) {
            var r = isHorizontal ? row : row + i;
            var c = isHorizontal ? col + i : col;
            if (r < 0 || r >= GRID_SIZE || c < 0 || c >= GRID_SIZE) return false;
    ...
    function getAITarget() {
        ...
        if (difficulty === 'easy') {
            ...
            for (var r = 0; r < GRID_SIZE; r++) {
                for (var c = 0; c < GRID_SIZE; c++) {
                    if (!gameState.playerBoard[r][c].hit && gameState.playerBoard[r][c].ship !== null) {
                        return { row: r, col: c };
                    }
                }
            }
            ...
            var probMap = calculateAIProbability();
            var maxProb = 0;
            var bestTargets = [];
            for (var r = 0; r < GRID_SIZE; r++) {
                for (var c = 0; c < GRID_SIZE; c++) {
                    if (!gameState.playerBoard[r][c].hit) {
                        if (probMap[r][c] > maxProb) {
                            maxProb = probMap[r][c];
                            bestTargets = [{ row: r, col: c }];
                        } else if (probMap[r][c] === maxProb && maxProb > 0) {
                            bestTargets.push({ row: r, col: c });
                        }
                    }
                }
            }
    ...
    function updateStats() {
        ...
        var totalEnemyShips = gameState.difficulty === 'insane' ? 2 : SHIPS.length;
        if (ps) ps.textContent = (SHIPS.length - gameState.enemyHits); 
        if (es) es.textContent = (totalEnemyShips - gameState.playerHits);
    ...
    function checkGameOver() {
        var totalEnemyShips = gameState.difficulty === 'insane' ? 2 : SHIPS.length;
        if (gameState.playerHits === totalEnemyShips) { 
    ...
    function showGameOver(playerWon) {
        ...
        if (playerWon) {
            ...
            appSettings.currentWinStreak++;
            if (appSettings.currentWinStreak > appSettings.bestWinStreak) {
                appSettings.bestWinStreak = appSettings.currentWinStreak;
            }
            if (!appSettings.fastestWin || gameState.shotsFired < appSettings.fastestWin) {
                appSettings.fastestWin = gameState.shotsFired;
            }
            var hitRate = gameState.shotsFired > 0 ? Math.round((gameState.hits / gameState.shotsFired) * 100) : 0;
            if (!appSettings.mostEfficientWin || hitRate > appSettings.mostEfficientWin) {
                appSettings.mostEfficientWin = hitRate;
            }
        } else {
            appSettings.gamesLost++;
            appSettings.currentWinStreak = 0;
        }
        saveSettings();
        setTimeout(function() {
            ...
            if (fss) fss.textContent = gameState.playerHits + ' / ' + totalEnemyShips;
            var stats = { 
                victory: playerWon, 
                shotsFired: gameState.shotsFired, 
                hitRate: hitRate, 
...
function resetAllData() {
    ...
    if (settingsScreen) settingsScreen.className = 'screen hidden';
    
    var confirmDiv = document.createElement('div');
    confirmDiv.id = 'reset-confirm-screen';
    confirmDiv.className = 'screen';
    confirmDiv.innerHTML = '<div class="menu-content"><h2>Reset All Data?</h2><p style="margin: 20px; font-size: 18px;">This will delete all achievements, stats, and settings. This cannot be undone!</p><button type="button" class="menu-btn" id="confirm-reset-yes">Yes, Reset Everything</button><button type="button" class="menu-btn" id="confirm-reset-no">Cancel</button></div>';
    
    document.getElementById('game-container').appendChild(confirmDiv);
    
    document.getElementById('confirm-reset-yes').onclick = function() {
        try {
            localStorage.removeItem('battleship_settings');
            appSettings.animationsEnabled = true;
            appSettings.unlockedAchievements = [];
            appSettings.gridSize = 10;
            appSettings.gamesWon = 0;
            appSettings.gamesLost = 0;
            appSettings.totalShipsSunk = 0;
            appSettings.totalShotsFired = 0;
            appSettings.totalHits = 0;
            appSettings.currentWinStreak = 0;
            appSettings.bestWinStreak = 0;
            appSettings.fastestWin = null;
            appSettings.mostEfficientWin = null;
            
            var conf = document.getElementById('reset-confirm-screen');
            if (conf && conf.parentNode) conf.parentNode.removeChild(conf);
            
            updateStatus('All data has been reset!');
            goBackToMenu();
        } catch (e) {
            updateStatus('Error resetting data');
            var conf = document.getElementById('reset-confirm-screen');
            if (conf && conf.parentNode) conf.parentNode.removeChild(conf);
            showSettings();
        }
    ...
    document.getElementById('confirm-reset-no').onclick = function() {
        var conf = document.getElementById('reset-confirm-screen');
        if (conf && conf.parentNode) conf.parentNode.removeChild(conf);
        showSettings();
    };
    ...
    function updateInsaneButton() {
        var insaneBtn = document.getElementById('insane-btn');
        if (!insaneBtn) return;
        var hardUnlocked = appSettings.unlockedAchievements.indexOf('dominator') !== -1;
        if (hardUnlocked) {
            insaneBtn.className = 'menu-btn';
        } else {
            insaneBtn.className = 'menu-btn hidden';
        }
    }
